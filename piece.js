...
class Move {
    ...
    draw() {
        ...
        ctx.beginPath();
        ctx.moveTo(
            this.piece.x * game.board.squaresize + game.board.squaresize / 2,
            (7 - this.piece.y) * game.board.squaresize +
                game.board.squaresize / 2
        );
        ctx.lineTo(
            this.x * game.board.squaresize + game.board.squaresize / 2,
            (7 - this.y) * game.board.squaresize + game.board.squaresize / 2
        );
        ctx.stroke();
...
class Piece {
    ...
    draw() {
        ctx.drawImage(
            this.img,
            this.x * game.board.squaresize,
            (7 - this.y) * game.board.squaresize,
            game.board.squaresize,
            game.board.squaresize
        );
    }
    ...
    drawAt(x, y) {
        ctx.drawImage(
            this.img,
            x - game.board.squaresize / 2,
            y - game.board.squaresize / 2,
            game.board.squaresize,
            game.board.squaresize
        );
    }
        ...
        if (this.player.king.isCheck.length) {
            for (let piece of this.player.king.isCheck) {
                for (let move of this.moves) {
                    ...
                    let testPlayer = this.isWhite
                        ? testBoard.white
                        : testBoard.black;
                    let threataningPiece = testBoard.getPieceAt(
                        piece.x,
                        piece.y
                    );
                    if (!threataningPiece) {
...
class Rook extends Piece {
    constructor(x, y, board, isWhite) {
        super(
            ...
            Rook,
            isWhite,
            (isWhite ? images.w : images.b).rook,
            pieceValues.rook
        );
...
class Queen extends Piece {
    constructor(x, y, board, isWhite) {
        super(
            ...
            Queen,
            isWhite,
            (isWhite ? images.w : images.b).queen,
            pieceValues.queen
        );
...
class Bishop extends Piece {
    constructor(x, y, board, isWhite) {
        super(
            ...
            Bishop,
            isWhite,
            (isWhite ? images.w : images.b).bishop,
            pieceValues.bishop
        );
...
class Knight extends Piece {
    constructor(x, y, board, isWhite) {
        super(
            ...
            Knight,
            isWhite,
            (isWhite ? images.w : images.b).knight,
            pieceValues.knight
        );
    ...
    recalculateMoves(checkCheck = true) {
        ...
        for (let vector of this.attackVectors) {
            ...
            if (
                ...
                x < 8 &&
                y < 8 &&
                (!pieceAt || pieceAt.isWhite != this.isWhite)
            ) {
                this.moves.push(new Move(x, y, this));
            }
...
class Pawn extends Piece {
    constructor(x, y, board, isWhite) {
        super(
            ...
            Pawn,
            isWhite,
            (isWhite ? images.w : images.b).pawn,
            pieceValues.pawn
        );
    }
    ...
    recalculateMoves(checkCheck = true) {
        ...
        let pieceAt = this.board.getPieceAt(
            this.x - 1,
            this.y + this.forwards()
        );
        if (pieceAt && pieceAt.isWhite != this.isWhite) {
            this.moves.push(
                new Move(this.x - 1, this.y + this.forwards(), this)
            );
        }
        ...
        if (!this.board.getPieceAt(this.x, this.y + this.forwards())) {
            ...
            if (
                !this.board.getPieceAt(this.x, this.y + this.forwards(2)) &&
                this.firstMove
            ) {
                this.moves.push(
                    new Move(this.x, this.y + this.forwards(2), this)
                );
            }
        }
...
class King extends Piece {
    constructor(x, y, board, isWhite) {
        super(
            ...
            King,
            isWhite,
            (isWhite ? images.w : images.b).king,
            pieceValues.king
        );
    ...
    recalculateMoves(vb = false) {
        this.moves = [];
        this.attacking = [];

        let x, y, pieceAt;

        let enemyAttackList = this.isWhite
            ? this.board.black.attacking
            : this.board.white.attacking;
        ...
        let backRank = this.isWhite ? 0 : 7;
        // Castling check

        // Queen's rook
        const queensRook = this.board.getPieceAt(0, backRank);
        if (
            // Neither king or rook has moved
            this.firstMove &&
            queensRook instanceof Rook &&
            queensRook.firstMove &&
            // No pieces inbetween
            !this.board.getPieceAt(1, backRank) &&
            !this.board.getPieceAt(2, backRank) &&
            !this.board.getPieceAt(3, backRank) &&
            // No threatened squares
            !this.enemy.getIsAttacking(1, backRank) &&
            !this.enemy.getIsAttacking(2, backRank) &&
            !this.enemy.getIsAttacking(3, backRank) &&
            !this.enemy.getIsAttacking(4, backRank)
        ) {
            this.moves.push(
                new Move(2, backRank, this, new Move(3, backRank, queensRook))
            );
        }

        // King's rook
        const kingsRook = this.board.getPieceAt(7, backRank);
        if (
            // Neither king or rook has moved
            this.firstMove &&
            kingsRook instanceof Rook &&
            kingsRook.firstMove &&
            // No pieces inbetween
            !this.board.getPieceAt(5, backRank) &&
            !this.board.getPieceAt(6, backRank) &&
            // No threatened squares
            !this.enemy.getIsAttacking(5, backRank) &&
            !this.enemy.getIsAttacking(6, backRank) &&
            !this.enemy.getIsAttacking(4, backRank)
        ) {
            this.moves.push(
                new Move(6, backRank, this, new Move(5, backRank, kingsRook))
            );
        }
